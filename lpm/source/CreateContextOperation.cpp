...
#define KC_COUNTING_ONLY 1 /* for now (temporary KC for QLP-SG) */

#ifndef KC_COUNTING_ONLY
...
#ifdef KC_COUNTING_ONLY
	{
		VERIFY(numPeriodsInclDummies == 1); // don't remove
		VERIFY(numStates == numStatesInclDummies);
		...
		for(ull stateIdx1 = 0; stateIdx1 < numStates; stateIdx1++)
		{
			...
			for(ull stateIdx2 = 0; stateIdx2 < numStates; stateIdx2++)
			{
				...
				transitionMatrixSum[transMatrixIdx] = count[transMatrixIdx];

				sum += transitionMatrixSum[transMatrixIdx];
			}
			...
			for(ull stateIdx2 = 0; stateIdx2 < numStates; stateIdx2++)
			{
				...
				transitionMatrixSum[transMatrixIdx] /= sum;
			}
		}
		...
		for(ull stateIdx1 = 0; stateIdx1 < numStates; stateIdx1++)
		{
			...
			{
				ull transMatrixIdx = GET_INDEX(stateIdx1, stateIdx2, numStatesInclDummies);
				steadyStateVector[stateIdx2] += count[transMatrixIdx];
			}
		}

		NORMALIZE_VECTOR(steadyStateVector, numStatesInclDummies);


		Free(varianceMatrix); varianceMatrix = NULL;

		Free(count); count = NULL;
		Free(transitionMatrix); transitionMatrix = NULL;
	}
#else
	...
	if(step > 1)
	{
		...
		for(ull stateIdx1 = 0; stateIdx1 < numStates; stateIdx1++)
		{
			for(ull stateIdx2 = 0; stateIdx2 < numStates; stateIdx2++)
			{
				...
				varianceMatrix[index] -= (transitionMatrixSum[index] * transitionMatrixSum[index]);
			}
		}
	}
...
bool CreateContextOperation::ReadLearningTraces(const vector<File*>& learningTracesFileVector, map<ull, vector<TraceVector> >& learningTraces) const
{
  // Bouml preserved body begin 00081F91
	...
	ull tpStartTm = tpInfo.partitioning->GetOffset(true);
	ull tpEndTm = tpStartTm + tpInfo.partitioning->GetLength() - 1;
	...
	foreach_const(vector<File*>, files, iter)
	{
		...
		while(file->IsGood()) // read all events from file and store them in 'traceSet'
		{
			...
			if(tm >= tpStartTm && tm <= tpEndTm){ traceSet->AddEvent(actualEvent); } // keep only events which fall within the time partitioning window
		...
		pair_foreach_const(map<ull, Trace*>, mapping, iterMapping)
		{
			...
			foreach_const(vector<Event*>, events, iterEvents) // fill in the proper trace vectors
			{
...
// This function is modified from original LPM code; it will not work with multiple time periods
bool CreateContextOperation::ComputeAggregateStatistics(File* tracesFile, File* locationsFile, double** outTransitionMatrix, double** outSteadyStateVector) const
{
	// Bouml preserved body begin 000DC791

	VERIFY(tracesFile != NULL && outSteadyStateVector != NULL && outTransitionMatrix != NULL);
	VERIFY(locationsFile != NULL);

	vector<File*> tracesFileVector; tracesFileVector.push_back(tracesFile);
	...
	VERIFY(locationsFile->IsGood() == true);

	// !!!!! NOT TIME DEPENDENT - WILL NOT WORK AS IS WITH TIME PERIODS
	VERIFY(numStatesInclDummies == numLoc);

	LineParser<double>* parser = LineParser<double>::GetInstance();

	ull loc = minLoc; vector<pair<double, double> > locsXYvec;
	while(locationsFile->IsGood())
	{
		string line = "";

		bool readOk = locationsFile->ReadNextLine(line);
		VERIFY(readOk == true);

		if(locationsFile->IsGood() == false && line.empty() == true) { break; }

		double locx = 0.0; double locy = 0.0;
		size_t pos = 0;
		bool parseOk = parser->ParseTwoFields(line, &locx, &locy, &pos, DEFAULT_FIELDS_DELIMITER);
		VERIFY(parseOk == true && pos == string::npos);

		locsXYvec.push_back(make_pair(locx, locy));
	...
	// count
	ull countdByteSize = numStatesInclDummies * numStatesInclDummies * sizeof(ull);
	double* countd = (double*)Allocate(countdByteSize);
	VERIFY(countd != NULL);
	memset(countd, 0, countdByteSize);

	const double minDist = 1.0;
	for(ull loc1 = minLoc; loc1 <= maxLoc; loc1++)
	{
		ull loc1idx = (loc1 - minLoc);
		double loc1x = locsXYvec[loc1idx].first;
		double loc1y = locsXYvec[loc1idx].second;

		for(ull loc2 = minLoc; loc2 <= maxLoc; loc2++)
		{
			ull loc2idx = (loc2 - minLoc);
			double loc2x = locsXYvec[loc2idx].first;
			double loc2y = locsXYvec[loc2idx].second;


			ull idx = GET_INDEX(loc1idx, loc2idx, numLoc);

			double dist = sqrt(pow((loc1x - loc2x), 2.0) + pow((loc1y - loc2y), 2.0));

			if(dist < minDist) { dist = minDist; }

			double prob = 1.0 / pow(dist, 2.0);
	...
	map<ull, vector<TraceVector> > tracesMap;
	bool readOk = ReadLearningTraces(tracesFileVector, tracesMap);
	VERIFY(readOk == true);
	...
	// add epsilon
	const double epsilon = 1.0 / (double)numLoc;
	for(ull stateIdx1 = 0; stateIdx1 < numStatesInclDummies; stateIdx1++)
	{
		for(ull stateIdx2 = 0; stateIdx2 < numStatesInclDummies; stateIdx2++)
		{ count[GET_INDEX(stateIdx1, stateIdx2, numStatesInclDummies)] += epsilon; }
	}
	...
	pair_foreach_const(map<ull, vector<TraceVector> >, tracesMap, iterMap)
	{
		const vector<TraceVector>& tracesVector = iterMap->second;

		for(size_t i = 0; i < tracesVector.size(); i++)
		{
			const TraceVector& tvec = tracesVector[i];
			ull* trace = tvec.trace;
	...
	for(ull stateIdx1 = 0; stateIdx1 < numStatesInclDummies; stateIdx1++)
	{
		for(ull stateIdx2 = 0; stateIdx2 < numStatesInclDummies; stateIdx2++)
		{
			...
			count[index] = alpha * count[index] + (1.0 - alpha) * countd[index];
		}
	}
	////////////////////////// ------------
	...
	ComputeSteadyStateVector(count, steadyStateVector);
	...
	if(numStates != numStatesInclDummies)
	{
			...
			for(ull loc1 = minLoc; loc1 <= maxLoc; loc1++)
			{
				...
				for(ull tp2 = minPeriod; tp2 <= maxPeriod; tp2++)
				{
					for(ull loc2 = minLoc; loc2 <= maxLoc; loc2++)
					{
						...
						transitionMatrix[subMatrixIdx] = count[fullMatrixIdx];
						sum += transitionMatrix[subMatrixIdx];
					}
				}
		...
		*outTransitionMatrix = transitionMatrix;
		*outSteadyStateVector = subSteadyStateVector;
	}
	else
	{
		*outTransitionMatrix =  count;
		*outSteadyStateVector = steadyStateVector;
	}

	return true;
	// Bouml preserved body end 000DC791
}
